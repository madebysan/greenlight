...
### Feature 1: Wizard Shell and Navigation
...
- Input: Stage 0 JSON
- Prompt engineering: JSON to structured markdown with scene-by-scene table
...
- Input: Stage 0 JSON
- Prompt engineering: JSON to 5 cross-referenced markdown tables
...
- Input: Stage 0 JSON
- Prompt engineering: JSON to narrative marketing document
...
- Input: Stage 0 JSON
- Prompt engineering: JSON to per-scene image generation prompts
...
- Input: Stage 0 JSON
- Prompt engineering: JSON to many diverse poster ideas
...
### Checkpoint First
1. At the START of every iteration, read checkpoint.json from the project root at /Users/san/Projects/script-to-production/checkpoint.json
2. Pick up EXACTLY where the last iteration left off, check current_phase, current_phase_step
3. If checkpoint.json does not exist yet, create it with the initial schema and start Phase 1
...
### Git Discipline
6. Git commit after every successful feature or fix: git add specific-files then git commit -m "feat: feature name"
...
Scaffold: npx create-next-app@latest . --typescript --tailwind --eslint --app --src-dir=false --import-alias="@/*" then install additional deps: npm install @anthropic-ai/sdk react-markdown remark-gfm
Type Check: npx tsc --noEmit
Dev Server: npm run dev
Test: npx vitest run (install vitest + @testing-library/react as devDependencies)

For shadcn/ui: npx shadcn@latest init then add components as needed with npx shadcn@latest add component-name

## Phase Execution Order

### Phase 1: Scaffold and Implement
- Create project structure if it does not exist
- Install dependencies
- Implement features one by one from the plan implementation order
- Type check after each feature
- Commit after each successful feature
- Skip to Phase 2 when all features are done (or deferred)

### Phase 2: Test and Fix
- Create basic smoke tests with Vitest + React Testing Library
- Run test suite
...
- Use Playwright to navigate every view/step of the wizard
- Capture: console errors, console warnings, uncaught exceptions, failed network requests
- For each issue found: diagnose root cause, fix (up to 3 attempts), re-run QA on affected view
...
- Analyze each screenshot for: layout breaks, overflow, clipping, alignment/spacing issues, missing content, contrast/readability
- Fix issues (up to 3 attempts each)
- Re-screenshot to verify fixes
- Save final screenshots to screenshots/ directory

### Phase 5: Security Audit
- Scan for hardcoded secrets and exposed credentials
- Verify .env.local is in .gitignore
- Run npm audit
- Check for XSS in user-rendered content (the markdown rendering)
- Verify env vars use NEXT_PUBLIC_ prefix only when intended for client
- Fix critical issues, log medium/low for report

### Phase 6: Final Verification + Report
- Run type checker one final time
...
- NEVER skip updating checkpoint.json, it is your memory across iterations
- ALWAYS commit working state before making changes
- If something is not working after 3 tries, DEFER and MOVE ON
- The goal is a WORKING v0, not a perfect product
- When in doubt, choose the simpler approach
- The working directory is /Users/san/Projects/script-to-production
- The ANTHROPIC_API_KEY is available via source ~/.zshrc, write it to .env.local as ANTHROPIC_API_KEY for Next.js to use
